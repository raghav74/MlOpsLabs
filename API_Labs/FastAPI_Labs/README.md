...
1. Training a Logistic Regression classifier on the Wine dataset.
2. Loading the trained model once at app startup.
3. Serving predictions through FastAPI and uvicorn.

## Setting up the lab

1. Create a Python 3.12 virtual environment (e.g. **fastapi_lab1_env**).
...
1. First step is to train a Logistic Regression classifier (this creates **`model/wine_model.pkl`**). To do this, move into **src/** folder with
    ```bash
    cd src
    ```
...
3. To serve the trained model as an API, run:
    ...
    ```
   The model is loaded once during app startup and then reused for all requests.
